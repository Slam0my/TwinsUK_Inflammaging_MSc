...
setwd("/users/k25046756/Twins_Project/data/protein_protein_solar")

#In /results there are separate directories for each job_chunk_
job_chunks <- list.files("/users/k25046756/Twins_Project/results/protein_protein_solar",
...
write.table(summary, "prot_summary_heritability.csv", sep="\t", row.names=FALSE, quote=FALSE)

#For viewing table for all 
summary_results<- read.delim("prot_summary_heritability.csv")
...
#Adding in the missing into summary csv 
missing_dir <- "/users/k25046756/Twins_Project/data/protein_protein_solar/missing"


missing_pairs <- list.dirs(missing_dir, full.names = TRUE, recursive = FALSE)
...
for(pair_dir in missing_pairs){
  ...
  if(file.exists(file)){
    ...
    if(length(h2_lines) >= 2) {
      h2r1 <- as.numeric(str_extract(h2_lines[1], "(?<=is )[0-9.Ee+]+"))
      h2r1_se_line <- lines[grepl("H2r.*Std\\. Error:", lines)]
      h2r1_se <- if(length(h2r1_se_line) > 0) as.numeric(gsub(".*Std\\. Error:\\s*([0-9.Ee+-]+).*", "\\1", h2r1_se_line[1])) else NA
      ...
      rhoE_line <- lines[grep("RhoE is", lines)]
      rhoE <- if(length(rhoE_line) > 0) as.numeric(str_extract(rhoE_line[1], "(?<=RhoE is )-?[0-9.Ee+]+")) else NA
      rhoE_p <- if(length(rhoE_line) > 0) as.numeric(str_extract(rhoE_line[1], "(?<=p = )[0-9.Ee+]+")) else NA
      rhoE_se_line <- lines[grepl("RhoE.*Std\\. Error:", lines)]
      rhoE_se <- if(length(rhoE_se_line) > 0) as.numeric(gsub(".*Std\\. Error:\\s*([0-9.Ee+-]+).*", "\\1", rhoE_se_line[1])) else NA
      
      rhoG_line <- lines[grep("RhoG is", lines)]
      rhoG <- if(length(rhoG_line) > 0) as.numeric(str_extract(rhoG_line[1], "(?<=RhoG is )-?[0-9.Ee+]+")) else NA
      rhoG_se_line <- lines[grepl("RhoG.*Std\\. Error:", lines)]
      rhoG_se <- if(length(rhoG_se_line) > 0) as.numeric(gsub(".*Std\\. Error:\\s*([0-9.Ee+-]+).*", "\\1", rhoG_se_line[1])) else NA
      
      rhoG_p0_line <- lines[grep("RhoG different from zero", lines)]
      rhoG_p0 <- if(length(rhoG_p0_line) > 0) as.numeric(str_extract(rhoG_p0_line[1], "([0-9.]+([eE][-+]?[0-9]+)?)$")) else NA
      
      rhoP_line <- lines[grep("Derived Estimate of RhoP is", lines)]
      rhoP <- if(length(rhoP_line) > 0) as.numeric(str_extract(rhoP_line[1], "-?[0-9.Ee+]+$")) else NA
      
      missing_summary <- rbind(missing_summary,
                               data.frame(trait1, trait2, h2r1, h2r1_se, h2r2, h2r2_se, rhoE, rhoE_p, rhoE_se, rhoG, rhoG_p0, rhoG_se, rhoP, stringsAsFactors = FALSE))
    }
  }
}

existing_results <- read.delim("/users/k25046756/Twins_Project/data/protein_protein_solar/prot_summary_heritability.csv")


final_combined_summary <- bind_rows(existing_results, missing_summary)

write.table(final_combined_summary, 
            "/users/k25046756/Twins_Project/data/protein_protein_solar/prot_summary_heritability_COMPLETE.csv", 
